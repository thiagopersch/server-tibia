...
function creatureSayCallback(cid, type, msg)
        ...
        if task_sys[task] then
            if getPlayerStorageValue(cid, task_sys[task].start) <= 0 then
                if getPlayerLevel(cid) >= task_sys[task].level then
                    ...
                    npcHandler:say("[Task System] Congratulations, you are now participating in the Task " .. task_sys[task].name .. " and must kill " .. task_sys[task].count .. " from this list: " .. getMonsterFromList(task_sys[task].monsters_list) .. ". " .. (#task_sys[task].items > 0 and "Oh, and please bring me " .. getItemsFromList(task_sys[task].items) .. " for me." or "") .. "", cid)
                else
                    npcHandler:say("Sorry, but you need to reach level " .. task_sys[task].level .. " to be able to participate in the Task " .. task_sys[task].name .. "!", cid)
                end
            else
                npcHandler:say("Sorry, but you are currently on the task " .. task_sys[task].name .. ". You can {deliver} if you've already finished.", cid)
            end
        else
    ...
    elseif isInArray({"diaria", "daili", "daily", "dayli", "diario"}, msg) then
        if getPlayerStorageValue(cid, task_sys_storages[6]) - os.time() > 0 then
            npcHandler:say("Sorry, you must wait until " .. os.date("%d %B %Y %X", getPlayerStorageValue(cid, task_sys_storages[6])) .. " to start a new daily task!", cid)
            return true
        ...
        npcHandler:say("[Daily Task System] Congratulations, you are now participating in the Task Daily " .. dtask.name .. " and should kill " .. dtask.count .. " monsters on this list: " .. getMonsterFromList(dtask.monsters_list) .. " to " .. os.date("%d %B %Y %X", getPlayerStorageValue(cid, task_sys_storages[6])) .. ". Good luck!", cid)
        ...
        if v then -- original task
            if getPlayerStorageValue(cid, v.start) > 0 then
                if getPlayerStorageValue(cid, task_sys_storages[3]) >= v.count then
                    ...
                    npcHandler:say("Thanks for your help Rewards: " .. (str == "" and "none" or "" .. str .. "") .. " for completing the task of " .. v.name, cid)
                    setPlayerStorageValue(cid, task_sys_storages[3], 0)
                    setPlayerStorageValue(cid, task_sys_storages[1], (task + 1))
                else
                    npcHandler:say("Sorry, but you haven't finished your task yet " .. v.name .. ". I need you to kill more " ..
                            (getPlayerStorageValue(cid, task_sys_storages[3]) < 0 and v.count or -(getPlayerStorageValue(cid, task_sys_storages[3]) - v.count)) .. " of these terrible monsters!", cid)
                end
            end
        end
        if k then -- daily task
            if getPlayerStorageValue(cid, task_sys_storages[7]) > 0 then
                if getPlayerStorageValue(cid, task_sys_storages[5]) >= k.count then
                    ...
                    npcHandler:say("Thanks for your help! Rewards: " .. (rst == "" and "none" or "" .. rst .. "") .. " for completing the task of " .. k.name, cid)
                    setPlayerStorageValue(cid, task_sys_storages[4], 0)
                    setPlayerStorageValue(cid, task_sys_storages[5], 0)
                    setPlayerStorageValue(cid, task_sys_storages[7], 0)
                else
                    npcHandler:say("Sorry, but you haven't finished your daily task yet. " .. k.name .. ". I need you to kill more " .. (getPlayerStorageValue(cid, task_sys_storages[5]) < 0 and k.count or - (getPlayerStorageValue(cid, task_sys_storages[5]) - k.count)) .. " of these monsters!", cid)
                end
            end
        end
    elseif msg == "no" then
